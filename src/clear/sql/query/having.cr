module Clear::SQL::Query::Having
  ...
  def having(template : String, **tuple)
    having(Clear::Expression::Node::Raw.new(Clear::SQL.raw(template, **tuple)))
  end
  ...
  def or_having(template : String, **named_tuple)
    or_having(Clear::Expression::Node::Raw.new(Clear::Expression.raw("(#{template})", **named_tuple)))
  end

  def or_having(template : String, *args)
    or_having(Clear::Expression::Node::Raw.new(Clear::Expression.raw("(#{template})", *args)))
  end
